...
def load_workbook(wb):
    ...
    for i in range(STARTING_ROW, nrows):
        if company_type != "" and (not is_bold(sheet.cell(i, 1)) and (not common.is_blank(sheet.cell(i, 1)))):
        ...
        elif is_bold(sheet.cell(i, 1)) or ( isinstance(sheet.cell(i, 1).value, str) 
            and ("державні" in sheet.cell(i, 1).value  or "господарські" in sheet.cell(i, 1).value )):
...
def main():
    ...
    OUTPUT_FILE = os.path.join(common.OUTPUT_FOLDER, 'coefficient_analysis')
    VARIABLES_FILE = 'sheet4.csv'
    STARTING_ROW = 7
    SHEET_NUMBER = 4
